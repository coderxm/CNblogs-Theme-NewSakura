	...
	class Silence {
		constructor() {
			this.defaluts = {
				...
				topImg: {
					homeTopImg: [
						"https://xxx.jpg",			//需要到侧边栏html添加修改
					],
					notHomeTopImg: [
						"https://img2018.cnblogs.com/blog/1646268/201908/1646268-20190806172418911-2037584311.jpg" //非首页顶部图
					]
				},
				topInfo: {
		...
		getMainMode() {
			...
			document.addEventListener('visibilitychange', function () {
				if (document.visibilityState == 'hidden') {
					clearTimeout(time);
					document.title = '你敢走了(＃°Д°)';   //可替换
				} else {
					document.title = '你终于回来了(。・∀・)ノ';  //可替换
		...
		buildCustomElements() {
			...
			$title.html($title.html().replace(`漂泊的流浪舟 | ${autherName}`, `${blogTitle}`));  //可替换
		...
		mainHeader() {
			...
			var header =
					...
					`       <h1 class="center-text glitch is-glitching Ubuntu-font" data-text="">${config.title}</h1>` +
		...
		setHomeSuiBiList() {
			...
			let author = $(this.cnblogs.publicProfile).find('a:eq(0)').html() 
			for (let i = article_list.length - 1; i >= 0; i--) {
				let time = $('.day').find('div.dayTitle')[i].textContent.replace('年', '-').replace('月', '-').replace('日', ''); 
				...
				let bianji = $('.day').find('div.postDesc')[i].firstElementChild.href; 
				let url
				let desc_img = article_list[i].getElementsByClassName('desc_img')[0];
				if (desc_img !== undefined) {
					url = desc_img.src; //
				} else {
					url = 'https://img2020.cnblogs.com/blog/2027366/202008/2027366-20200807134055773-1108692656.jpg'     //文章配图这里改，可替换
				}
				let html = `<div class="post post-list-thumb post-list-show">` +
						...
						`     <div class="post-meta"> <span><i class="iconfont icon-attention"></i>${readNum} 热度</span> <span class="comments-number"><i class="iconfont icon-mark"></i>${comNum} 条评论</span> <span><i class="iconfont icon-cc-user"></i><a href="#"></a>${author}</span></div>` +  //可替换
		...
		postHeader() {
			...
			if (window.location.href.indexOf('articles') === -1) {
				var header =                                                                     //链接 头像 链接 均可替换
						`<p class="entry-census"><span><a href="https://www.cnblogs.com/coderma/"><img src="https://images.cnblogs.com/cnblogs_com/coderma/1820127/o_200803093536logo.png"></a></span><span><a href="https://www.cnblogs.com/coderma//">流浪舟</a></span><span class="bull">·</span>${post_date}<span class="bull">·</span>${post_view_count} 次阅读</p>`;
